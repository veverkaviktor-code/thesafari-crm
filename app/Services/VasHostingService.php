...
class VasHostingService
{
    ...
    /**
     * Activate a domain on the server (vas-hosting REST API).
     * Creates hosting space for the domain on sss06.vas-server.cz.
     *
     * Returns ['success' => true] or ['success' => false, 'message' => string, 'status' => int]
     */
    public function activateDomainOnServer(string $domainName): array
    {
        try {
            ...
            if ($response->successful()) {
                Log::info('VasHosting: doména aktivována na serveru', [
                    'domain' => $domainName,
            ...
            Log::warning('VasHosting: aktivace domény selhala', [
                'domain' => $domainName,
                'status' => $response->status(),
                'body'   => $body,
            ]);
            return [
                'success' => false,
                'message' => $body['message'] ?? 'API vrátilo chybu ' . $response->status(),
                'status'  => $response->status(),
            ];
        } catch (\Exception $e) {
            Log::error('VasHosting: chyba při aktivaci domény', [
                'domain' => $domainName,
                'error'  => $e->getMessage(),
            ]);
            return [
                'success' => false,
                'message' => 'Síťová chyba: ' . $e->getMessage(),
                'status'  => 0,
            ];
        }
    }
